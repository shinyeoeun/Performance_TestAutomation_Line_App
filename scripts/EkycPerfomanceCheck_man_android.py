...
class AndroidEkycPerfomanceTestMan(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        time.sleep(2)
        self.driver.find_element_by_id("com.linecorp.yuki:id/manual_button").click()

    def test001_check_sameface_man_01_normal(self):
        """본인인증 촬영 테스트 (男): 신분증과 동일한 인물 / 일반사진 → 인식률 점수가 0.4이상이고 same face토스트가 표시될 것"""
    ...
    def test002_check_sameface_man_01_bright(self):
        """본인인증 촬영 테스트 (男): 신분증과 동일한 인물 / 밝게 촬영된 사진 → 인식률 점수가 0.4이상이고 same face토스트가 표시될 것"""
        image_path = "ekyc/testData/selfie/selfie_man_01_2.jpg"
        android_utils.takeSelfie(self, image_path)

    def test003_check_sameface_man_01_dark(self):
        """본인인증 촬영 테스트 (男): 신분증과 동일한 인물 / 어둡게 촬영된 사진 → 인식률 점수가 0.4이상이고 same face토스트가 표시될 것"""
    ...
    def test054_check_sameface_man_16_dark(self):
        """본인인증 촬영 테스트 (男): 신분증과 동일한 인물 / 어둡게 촬영된 사진 → 인식률 점수가 0.4이상이고 same face토스트가 표시될 것"""
