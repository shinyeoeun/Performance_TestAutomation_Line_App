...
from sticker.AndroidStressTest import StressTestCase
...
class RunAllScript(object):
    ...
    def __init__(self):
        ...
        self.test_case_path = "."
        self.title = "Yuki SDK Automation TestReport-Android"
        self.description = "yuki SDK 리그레션테스트케이스 스크립트 수행결과레포트"

    def run(self):
        # ToDo 실행 스크립트 지정
        test_suite = unittest.TestSuite()
        test_suite.addTests(unittest.TestLoader().loadTestsFromTestCase(StressTestCase))
        ...
        runner = HTMLTestReport.HTMLTestRunner(stream=fp, title=self.title, description=self.description, tester="LFK: SHIN YEOEUN", build="2.17.1.1584")
